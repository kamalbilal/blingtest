...
gulp.task('injectCommonAssets', function () {
    return gulp.src('./**/*.html')
        .pipe(inject(gulp.src(['assets/css/style.css', 'assets/vendors/js/vendor.bundle.base.js'], { read: false }), { relative: true }))
        .pipe(gulp.dest('.'));
});
...
gulp.task('inject', gulp.series('injectPartial', 'injectCommonAssets', 'replacePath'));
...
gulp.task('copyAddonsStyles', function () {
    ...
    var aStyle4 = gulp.src(['./node_modules/owl.carousel/dist/assets/owl.carousel.min.css'])
        .pipe(gulp.dest('./assets/vendors/owl.carousel/dist/assets'));
    var aStyle5 = gulp.src(['./node_modules/owl.carousel/dist/assets/owl.theme.default.min.css'])
        .pipe(gulp.dest('./assets/vendors/owl.carousel/dist/assets'))
    return merge(aStyle1, aStyle2, aStyle3, aStyle4, aStyle5);
});

/*sequence for building vendor scripts and styles*/
gulp.task('bundleVendors', gulp.series('clean:vendors', 'buildBaseVendorScripts', 'copyAddonsStyles', 'copyAddonsScripts'));
